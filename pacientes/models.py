...
class Tarefas(models.Model):
    ...
    tafera = models.CharField(max_length=255)
...
class Consultas(models.Model):
    humor = models.PositiveBigIntegerField()
    ...
    def views(self):
        total_views = Visualizacoes.objects.filter(consulta=self).count()
        unique_views = Visualizacoes.objects.filter(consulta=self).values('ip').distinct().count()
        return f"{total_views} - {unique_views}"
    @property
...
class Visualizacoes(models.Model):
    ...
    def __str__(self):
        return f"Consulta {self.consulta.id} - IP: {self.ip}"
